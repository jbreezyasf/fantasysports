...
- **PROVEN:** The owner-confirmed rule now covers the entire roster transaction boundary. Production `roster_asset_game_has_started` resolves the active competition week from the official schedule and locks a rostered starter or bench asset once that asset's own game begins, while canceled/postponed games remain movable. `submit_waiver_claim` rejects a started corresponding drop immediately, `process_due_waivers` rechecks at award time without aborting other claims, and the roster-entry trigger protects direct releases and free-agent swaps.
- **PROVEN:** Rollback-only production acceptance checks rejected James Cook III as a direct roster drop, rejected clearing his started RB1 slot, allowed a not-started starter to move to the bench with exactly one audit row, and left Cook rostered at RB1 afterward. A separate rollback-only authenticated waiver test rejected Cook as the corresponding drop and persisted no claim.
- **PROVEN:** The owner-approved one-time JLC Week 2 correction restored James Cook III to RB1, kept Christian McCaffrey rostered on the bench, recomputed the matchup, and recorded restoration audit `6d2afb44-f76a-4e33-99d8-380a117b977f`. Baltimore D/ST is owner-confirmed intentional and remains unchanged. The reported Kyren Williams / Jadarian Price duplicates were an iOS scrolling-screenshot stitching artifact; the live lineup did not contain duplicate athlete assignments.
...
>
> **Current posture:** RECONCILED THROUGH READ-ONLY INSPECTION (2026-08-26) PLUS EXECUTED QA RUNS (2026-08-30 / 2026-08-31), REVIEWED 2026-09-04, WITH DEPLOYED-PRODUCTION RELEASE-HARDENING EVIDENCE ADDED 2026-09-08. Several gameplay flows have now been executed against production data and the deployed production app; no gate has yet satisfied every full-season acceptance criterion.

## September 8 Beta Draft Release Evidence

This update covers the September 8 release-hardening sprint for the real 10-manager friend beta draft. It does not mark the full product beta gate PASS, because the actual beta league preflight and controlled invite/signup smoke test are still not proven.

- **PROVEN:** On 2026-09-08, local `main`, `origin/main`, and Vercel production deployment `dpl_BwMW5BUSi2X71piSakxsRj5tnotQ` all pointed at commit `13117c6459fecbbdaa11560bff43390df2b17097`. Production URL tested: `https://www.bigexecfs.com`. Supabase project: `njjiqdqhmcbxblwhfade`.
- **PROVEN:** Production draft infrastructure exists: `drafts`, `draft_picks`, `draft_queues`, `draft_corrections`; queue/timer/autopick/pause/undo RPCs; active `big-exec-process-draft-autopicks` cron; realtime publication membership for `drafts`, `draft_picks`, and `draft_queues`.
- **PROVEN:** `QA_APP_URL=https://www.bigexecfs.com npm run qa:auth:save` saved all ten controlled QA actor browser sessions after repairing only the controlled QA Manager01 auth credential. No real beta participant data was modified.
- **PROVEN:** Deployed production full-draft dress rehearsal passed. Evidence: `qa-artifacts/2026-09-08_deployed-full-draft/DRAFT_QA.md`, league `add82581-5e75-4c90-b112-adab7471e4f8`, draft `ea5b1afd-bcb9-4d92-bbfb-3f28527856a1`. Results: 150/150 made picks, 150 unique drafted assets, 10 legal completed rosters, autopick observed, duplicate drafted asset rejected, pause/resume enforced, commissioner undo executed, desktop/mobile Draft Room screenshots captured, zero console/network errors in captures, and no manual database repair.
- **PROVEN:** Deployed production post-draft team-management QA passed. Evidence: `qa-artifacts/2026-09-08_deployed-post-draft-transactions/TRANSACTIONS_QA.md`. Results: lineup edit succeeded, cross-manager lineup write blocked, Free Agency/waiver flow created expected hold and competing waiver outcomes, accepted trade moved both assets atomically, invalid trade rejected, trade deadline rejected new trade, and private trade messages were protected. One expected non-participant private-trade route returned 404.
- **PROVEN with latency caveat:** Deployed realtime/reconnect QA passed functionally. Evidence: `qa-artifacts/2026-09-08_deployed-draft-realtime/DRAFT_REALTIME_QA.md`, 10 pass, 0 fail, 1 slow. Realtime propagation beat the 15-second polling fallback and reconnect recovered state. One propagation check exceeded the strict 8-second target (commissioner desktop 10,275ms, manager mobile 13,789ms), so live-draft monitoring should watch propagation latency.
- **PROVEN:** New read-only release commands exist: `npm run beta:draft:preflight -- --league=<league-id>` and `npm run ops:draft:health -- --league=<league-id>`. The preflight command defaults to inspection only and reports invitation counts without emails/secrets. The health command reports draft status, current pick, current manager, deadline, pause state, last pick, total picks, last autopick, recent correction/undo, roster-count anomalies, duplicate drafted assets, and expired-deadline anomaly.
- **PROVEN:** Fresh QA league `7dd3f39b-53b5-4420-bc37-b9635c9e31fa` preflight ended exactly `BETA DRAFT PREFLIGHT: PASS`.
- **UNVERIFIED:** The actual friend beta league id was not identified from production metadata without guessing; therefore the actual beta league preflight has not yet been run.
- **UNVERIFIED:** Controlled production invite -> account -> league smoke test has not yet been executed.
- **PROVEN:** BALDONTLIE NFL games ingestion was added using the existing provider path and official `GET /nfl/v1/games` parameters. Production now contains 272 2026 regular-season `real_games` rows; Week 1 has 16 rows, first kickoff `2026-09-10T00:20:00+00:00`, last kickoff `2026-09-15T00:15:00+00:00`. Repeat full sync inserted 0 rows, updated 272 rows, and left duplicate provider ids at 0.
- **PROVEN:** `/ops` data-health reads were fixed to use canonical `real_games.state`, not nonexistent `real_games.status`.
...
- **PROVEN:** On 2026-09-05 finish branch `finish/big-exec-voice-tts-animation`, the authenticated header Ask GM control is wired to the read-only Assistant GM gateway through `apps/web/app/components/assistantGmActions.ts`. Automated verification passed: `npm test --workspace @fantasy-all-sports/web` (50 files, 292 tests), `npm run test:a11y --workspace @fantasy-all-sports/web` (2 files, 8 tests), `npm run build --workspace @fantasy-all-sports/web`, and serial `npm run typecheck --workspace @fantasy-all-sports/web`.
...
- **UNVERIFIED:** Authenticated browser/microphone/TTS QA, production deployment verification, and VoiceOver/TalkBack device testing have not been executed for the 2026-09-05 Ask GM header wiring.
