...
**P0 containment 2026-09-18 — bidirectional lineup locks:** Production evidence confirmed `set_lineup_slot` rejected a started incoming player but allowed a started slot occupant to be removed, immediately orphaning accrued fantasy points from the matchup total. Migration `20260918005608_bidirectional_lineup_lock_and_audit.sql` is applied in production. Both RPC signatures now serialize franchise-week lineup writes, reject started assets on either side of a move (including final, delayed, suspended, and provider-unknown states), preserve canceled/postponed flexibility, retain ownership and roster validation, and record every successful move in `lineup_move_audit`. A rollback-only authenticated production rehearsal proved started-outgoing rejection with the original row unchanged, started-incoming rejection, an unlocked move succeeding, and audit insertion. The Team page implementation now calculates locks from the official week schedule, disables Change for locked starters with an accessible explanation, removes locked assets from replacement choices, and labels locked bench assets. The reported JLC Week 2 lineup remains in its observed state pending explicit approval for the direct production restoration of James Cook III to RB1; containment and product deployment do not require that separate data correction.
