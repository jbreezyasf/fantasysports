-- Prefer primary paid provider draft values without double-counting fallback rows.
-- balldontlie is primary. Existing fallback score imports remain useful only
-- when balldontlie has no row for the same asset and season.
