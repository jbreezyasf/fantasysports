...
const managers = Array.from({ length: 8 }, (_, index) => ({ label: `Manager0${index + 1}`, email: `juanita.brazziel+qa-manager-0${index + 1}@gmail.com` }));
const config = { schemaVersion: 1, testId: 'BIG_EXEC_STRESS_SEASON_2026', sport: 'pro_football', seasonYear: 2026, leagueId: '11111111-1111-4111-8111-111111111111', leagueSeasonId: '33333333-3333-4333-8333-333333333333', humanManagerEmails: ['juanita.brazziel@gmail.com','j_brazziel@yahoo.com'], draft: { startsAt: '2026-09-13T01:30:00.000Z', timezone: 'America/Chicago' }, disabledAt: '2027-03-01T00:00:00Z', syntheticManagers: managers };
...
test('mixed draft attendance is deterministic and includes autopick', () => {
  const modes = new Set(Array.from({ length: 150 }, (_, pick) => participationFor({ testId: config.testId, actorLabel: `Manager0${(pick % 8) + 1}`, overallPick: pick + 1 }).mode));
