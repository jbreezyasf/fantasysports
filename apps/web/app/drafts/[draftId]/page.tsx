  ...
  return (
    <main className="draftRoom draftRoomEvent">
      ...
      {draft.status !== 'completed' && <>
        ...
        <section className="draftOrderStrip" id="draft-board" aria-labelledby="draft-order-heading"><div><span>DRAFT ORDER</span><strong id="draft-order-heading">Snake board</strong></div><div className="draftOrderRail">{(seasonFranchises??[]).map(sf=>{const franchise=firstRef(sf.franchises as FranchiseRef|FranchiseRef[]|null);return <article className={`${sf.id===current?.season_franchise_id?'isOnClock':''} ${sf.id===mySeasonFranchise?.id?'isMine':''}`} key={sf.id}><span>{sf.draft_position}</span><div><strong>{franchise?.abbreviation??`P${sf.draft_position}`}</strong><small>{franchise?.name??'Franchise'}</small></div></article>;})}</div></section>
