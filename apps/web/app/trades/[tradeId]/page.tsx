...
export default async function TradeRoomPage({params,searchParams}:{params:Promise<{tradeId:string}>;searchParams:Promise<{error?:string;resolved?:string}>}){
 ...
 const {data:season}=await supabase.from('league_seasons').select('league_id').eq('id',trade.league_season_id).maybeSingle(); if(!season) notFound();
 ...
 <section className="panel"><p className="eyebrow">NEGOTIATION</p><div className="weekStack">{(messages??[]).map(m=><article className="weekCard" key={m.id}><div className="weekHeader"><strong>{names.get(m.user_id)??'Manager'}</strong><small>{new Date(m.created_at).toLocaleString('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}</small></div><p className="lede">{m.body}</p></article>)}{!messages?.length&&<p className="lede">No messages yet.</p>}</div>{trade.status==='proposed'&&<form className="authForm" action={postTradeRoomMessage}><input type="hidden" name="trade_id" value={tradeId}/><label>Private message<textarea name="body" required maxLength={1000} rows={3} placeholder="Make the case, counter, or close the deal."/></label><button className="secondary">Send Private Message</button></form>}</section>
 {trade.status==='proposed'&&<section className="panel"><p className="eyebrow">DECISION</p><div className="actions">{isRecipient&&<><form action={resolveTradeAction}><input type="hidden" name="trade_id" value={tradeId}/><input type="hidden" name="trade_action" value="accept"/><button className="primary">Accept Trade</button></form><form action={resolveTradeAction}><input type="hidden" name="trade_id" value={tradeId}/><input type="hidden" name="trade_action" value="reject"/><button className="secondary">Reject</button></form></>}{isProposer&&<form action={resolveTradeAction}><input type="hidden" name="trade_id" value={tradeId}/><input type="hidden" name="trade_action" value="cancel"/><button className="secondary">Cancel Offer</button></form>}</div></section>}
 </main>;
}
