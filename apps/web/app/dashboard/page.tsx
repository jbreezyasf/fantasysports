...
export default async function DashboardPage() {
  ...
  const [memberResult, profileResult, inviteResult, ownershipResult] = await Promise.all([
    supabase.from('league_members').select('league_id,role,fantasy_leagues(id,name,created_at)').eq('user_id', user.id).order('joined_at', { ascending: false }),
    supabase.from('user_profiles').select('display_name').eq('user_id', user.id).maybeSingle(),
    userEmail ? supabase.from('league_invites').select('id,invite_token,email,status,expires_at').eq('status','pending').eq('email', userEmail).order('created_at', { ascending:false }) : Promise.resolve({ data: [], error: null }),
  ...
  const inviteNameResults = inviteResult.data?.length
    ? await Promise.all(inviteResult.data.map(invite => supabase.rpc('get_public_league_invite', { p_invite_token: invite.invite_token })))
    : [];
  const invites: DashboardInvite[] = (inviteResult.data ?? []).map((invite, index) => {
    const publicInvite = Array.isArray(inviteNameResults[index]?.data) ? inviteNameResults[index].data[0] as { league_name?: string | null } | undefined : null;
    return { ...invite, league_name: publicInvite?.league_name ?? null };
  });
  ...
  const dataError = memberResult.error?.message || profileResult.error?.message || inviteResult.error?.message;
      ...
      {!!invites?.length && (
        <section className="frontOfficeSection">
          ...
          <div className="leagueTileGrid">
            {invites.map((invite) => {
              const leagueName = invite.league_name;
              return (
          ...
          {(leagues ?? []).map((league, index) => {
            return (
              <a className="leagueTile" key={league.id} href={`/leagues/${league.id}`}>
                <div className="leagueTileTop"><SportIdentity code="pro-football" displayName="Pro Football" compact/><b>{index===0?'LATEST':'LEAGUE'}</b></div>
