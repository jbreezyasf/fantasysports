...
export default async function BigExecMobileNav({leagueId}:{leagueId:string}){
  ...
  const item=(label:string,href?:string)=>href?<a href={href}>{label}</a>:<span aria-disabled="true">{label}</span>;
  return <nav className="mobileGameNav" aria-label="Big Exec primary navigation">
    {item('Home','/dashboard')}
    {item('Team',franchiseId?`/franchises/${franchiseId}/team`:undefined)}
    {item('Matchup',matchupId?`/matchups/${matchupId}`:undefined)}
