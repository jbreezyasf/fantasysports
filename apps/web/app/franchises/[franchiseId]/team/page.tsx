...
export default async function TeamPage({ params, searchParams }: { params: Promise<{ franchiseId: string }>; searchParams: Promise<{ week?: string; error?: string; lineup_status?: string; lineup_slot?: string; lineup_asset?: string; integrity_status?: string; integrity_error?: string }> }) {
  ...
  return <main>
    ...
    <section className="panel benchPanel" aria-labelledby="bench-heading"><div className="lineupSectionHeading"><div><p className="eyebrow">BENCH</p><h2 id="bench-heading">Roster reserves</h2></div><span>{(roster ?? []).filter(asset => !starterAssetIds.has(asset.athlete_id ?? asset.real_team_id)).length} players</span></div><div className="playerList">{(roster ?? []).filter(asset => !starterAssetIds.has(asset.athlete_id ?? asset.real_team_id)).map(asset => <div className="playerRow" aria-label={describeAssetForScreenReader(asset,'bench')} key={asset.id}><div><span>BENCH</span><strong>{labelForAsset(asset)}</strong><small className="srOnly">Valid move actions are available in matching starter slots above.</small></div></div>)}{!roster?.length && <p className="errorNotice">No roster yet. Players appear here as soon as the draft is completed.</p>}</div></section>
  </main>;
}
