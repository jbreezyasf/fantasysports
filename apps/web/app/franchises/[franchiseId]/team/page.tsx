...
export default async function TeamPage({ params, searchParams }: { params: Promise<{ franchiseId: string }>; searchParams: Promise<{ week?: string; error?: string; lineup_status?: string; lineup_slot?: string; lineup_asset?: string; integrity_status?: string; integrity_error?: string }> }) {
  ...
  const [{ data: roster }, { data: lineup }, { data: stadium }, { data: pendingReviews }, { data: playerScores }, { data: teamScores }] = await Promise.all([
    ...
    supabase.from('fantasy_player_scores').select('athlete_id,points').eq('league_season_id',currentLeagueSeason.id).eq('week',week),
    supabase.from('fantasy_team_scores').select('real_team_id,points').eq('league_season_id',currentLeagueSeason.id).eq('week',week)
  ]);
  ...
  return <main>
    ...
    <section className="panel lineupControlPanel"><p className="eyebrow">LINEUP CONTROL</p><h2>Set your starters.</h2><p className="lede">Open a position only when you want to make a change. Players lock when their game begins.</p>{query.error && <p className="errorNotice" role="alert">{query.error}</p>}{query.lineup_status==='set'&&<p className="successNotice" role="status">{lineupMoveConfirmation(query.lineup_asset??'Selected player',query.lineup_slot??'lineup slot',week)}</p>}<div className="actions">{week > 1 && <a className="secondary" href={`/franchises/${franchiseId}/team?week=${week-1}`}>← Week {week-1}</a>}{week < 18 && <a className="secondary" href={`/franchises/${franchiseId}/team?week=${week+1}`}>Week {week+1} →</a>}<a className="secondary" href={`/franchises/${franchiseId}/stadium`}>View My Stadium</a></div></section>
    <section className="panel lineupTablePanel" aria-labelledby="starters-heading"><div className="lineupSectionHeading"><div><p className="eyebrow">STARTERS</p><h2 id="starters-heading">Week {week} lineup</h2></div><div className="lineupWeekTotal"><strong>{starterPoints.toFixed(2)}</strong><small>WEEK {week} POINTS</small><span>{starterAssetIds.size}/9 SET</span></div></div><div className="lineupGrid">{slots.map(([slot,slotIndex,label]) => { const current = lineupMap.get(`${slot}:${slotIndex}`); const currentRoster = roster?.find(r => (current?.athlete_id && r.athlete_id===current.athlete_id) || (current?.real_team_id && r.real_team_id===current.real_team_id)); const currentLabel = currentRoster ? labelForAsset(currentRoster) : undefined; const eligible = (roster ?? []).filter(r => { if (r.real_team_id) return slot==='DST'; const athlete = Array.isArray(r.athletes) ? r.athletes[0] : r.athletes as {position?:string}|null; const pos = athlete?.position; if (slot==='FLEX') return ['RB','WR','TE'].includes(pos ?? ''); return pos===slot; }); return <article className="lineupSlot" aria-label={describeLineupSlot(label,currentLabel)} key={`${slot}-${slotIndex}`}><span>{label}</span><strong>{currentRoster ? labelForAsset(currentRoster) : 'Empty slot'}</strong><b className="lineupAssetPoints">{pointsForAsset(currentRoster).toFixed(2)} <small>PTS</small></b>{currentRoster&&<small className="srOnly">{describeAssetForScreenReader(currentRoster,'starter',label)}</small>}{!!eligible.length && <details className="lineupChange"><summary>Change</summary><div className="slotChoices" aria-label={`Move eligible players to ${label}`}>{eligible.slice(0,12).map(asset => { const assetLabel=labelForAsset(asset); return <LineupMoveForm key={asset.id} seasonFranchiseId={seasonFranchise.id} franchiseId={franchiseId} week={week} slot={slot} slotIndex={slotIndex} slotLabel={label} assetLabel={assetLabel} athleteId={asset.athlete_id} realTeamId={asset.real_team_id} buttonLabel={lineupMoveButtonLabel(assetLabel,label,week)}/>; })}</div></details>}</article>; })}</div></section>
    ...
    <section className="panel benchPanel" aria-labelledby="bench-heading"><div className="lineupSectionHeading"><div><p className="eyebrow">BENCH</p><h2 id="bench-heading">Roster reserves</h2></div><div className="lineupWeekTotal"><strong>{benchPoints.toFixed(2)}</strong><small>POINTS LEFT ON BENCH</small><span>{benchAssets.length} PLAYERS</span></div></div><div className="playerList">{benchAssets.map(asset => <div className="playerRow benchScoreRow" aria-label={`${describeAssetForScreenReader(asset,'bench')} ${pointsForAsset(asset).toFixed(2)} points in week ${week}.`} key={asset.id}><div><span>BENCH</span><strong>{labelForAsset(asset)}</strong><small>Week {week}</small></div><b className="lineupAssetPoints">{pointsForAsset(asset).toFixed(2)} <small>PTS</small></b></div>)}{!roster?.length && <p className="errorNotice">No roster yet. Players appear here as soon as the draft is completed.</p>}</div></section>
  </main>;
}
