...
const initialLineupActionState: LineupActionState = { status: 'idle', message: '' };
...
export function LineupMoveForm(props: Props) {
  ...
  return <form action={action} className="lineupMoveForm">
    ...
    {state.message && <span className="lineupMoveStatus" role={state.status === 'error' ? 'alert' : 'status'}>{state.message}</span>}
  </form>;
}
