...
export default async function FranchiseLayout({children,params}:{children:React.ReactNode;params:Promise<{franchiseId:string}>}){
  ...
  return <><SkipLink/><BigExecAppHeader leagueId={franchise.league_id} voiceGmEnabled={isVoiceFeatureEnabled('voice_gm')}/><MainContent>{children}</MainContent><BigExecMobileNav leagueId={franchise.league_id}/></>;
}
