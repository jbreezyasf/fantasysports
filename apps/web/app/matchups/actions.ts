...
function fallbackOptions(tone:Tone,f:Facts):string[] {
  ...
  if(tone==='respect') return f.requesterWon ? [`GG, ${f.loserName}. ${score}. That was a real one.`,`Respect to ${f.loserName}. Week ${f.week} made us earn every point.`,`Good battle. ${f.margin.toFixed(2)} points decided it. See you next time.`] : [`GG, ${f.winnerName}. You earned Week ${f.week}.`,`Respect. ${score}. We’ll see you again.`,`That one belongs to ${f.winnerName}. No excuses — good game.`];
  if(tone==='playful') return f.requesterWon ? [`Scoreboard submitted the paperwork: ${score}.`,`Week ${f.week} meeting adjourned. Margin: ${f.margin.toFixed(2)}.`,`The front office reviewed the numbers. We’ll be keeping this win.`] : [`The scoreboard has been placed under appeal. Results: ${score}.`,`Fine. ${f.winnerName} gets this meeting. Next agenda item: revenge.`,`Week ${f.week} has been deleted from my calendar. See you next time.`];
  if(tone==='petty') return f.requesterWon ? [`You had all week and still finished ${f.margin.toFixed(2)} points short. That’s tough.`,`I brought a lineup. ${f.loserName} brought a learning experience.`,`The final score is public record. I won’t embarrass you by repeating it twice.`] : [`Enjoy the screenshot, ${f.winnerName}. It has an expiration date.`,`One win and suddenly everybody has Wi-Fi. I’ll remember this.`,`Congratulations on surviving Week ${f.week}. The rematch file is already open.`];
  return f.requesterWon ? [`That wasn’t a matchup. That was a quarterly performance review. ${score}.`,`I asked for competition and got a case study. ${f.margin.toFixed(2)}-point margin.`,`The scoreboard closed the argument before the Locker Room could start one.`] : [`Frame the score, ${f.winnerName}. You’re going to need the memory later.`,`You got Week ${f.week}. I’m keeping the receipts and the schedule.`,`Enjoy the victory lap. The franchise remembers everything.`];
}
...
function extractResponseText(payload:unknown):string|null {
  ...
  if(Array.isArray(p.output)) for(const item of p.output as Array<{content?:unknown}>){ if(!Array.isArray(item?.content)) continue; for(const part of item.content as Array<{text?:unknown}>){ if(typeof part?.text==='string') return part.text; } }
  return null;
}
...
export async function generatePostgameTalk(formData:FormData) {
  ...
  const matchupId=String(formData.get('matchup_id')??''); const tone=String(formData.get('tone')??'playful').toLowerCase() as Tone;
  ...
  const [{data:sfs},{data:owners}]=await Promise.all([supabase.from('season_franchises').select('id,franchise_id,franchises(name)').in('id',[matchup.home_season_franchise_id,matchup.away_season_franchise_id]),supabase.from('franchise_owners').select('franchise_id,user_id').eq('user_id',user.id).is('ends_on',null)]);
  ...
  const ownedIds=new Set((owners??[]).map(o=>o.franchise_id)); const requesterSf=sfs?.find(sf=>ownedIds.has(sf.franchise_id));
  ...
  const homeName=nameFor(matchup.home_season_franchise_id),awayName=nameFor(matchup.away_season_franchise_id),winnerName=nameFor(matchup.winner_season_franchise_id);
  ...
  for(const body of generated.options){ const {error:recordError}=await supabase.rpc('record_generated_message',{p_matchup_id:matchupId,p_tone:tone,p_body:body,p_provider:generated.provider}); if(recordError) redirect(`/matchups/${matchupId}?error=${encodeURIComponent(recordError.message)}`); }
  revalidatePath(`/matchups/${matchupId}`); redirect(`/matchups/${matchupId}?talk=${tone}`);
}

export async function postGeneratedTalk(formData:FormData) {
  const supabase=await createClient(); const {data:{user}}=await supabase.auth.getUser(); if(!user) redirect('/login');
  const matchupId=String(formData.get('matchup_id')??''),messageId=String(formData.get('message_id')??''),body=String(formData.get('body')??'').trim();
  const {data,error}=await supabase.rpc('post_generated_message',{p_message_id:messageId,p_body:body||null}); if(error) redirect(`/matchups/${matchupId}?error=${encodeURIComponent(error.message)}`);
  const leagueId=(data as {league_id?:string}|null)?.league_id; if(!leagueId) redirect(`/matchups/${matchupId}?error=${encodeURIComponent('Could not post to Locker Room')}`);
  revalidatePath(`/leagues/${leagueId}/locker-room`); redirect(`/leagues/${leagueId}/locker-room`);
}
...
export async function buildArcadeRecap(formData:FormData) {
  ...
  const matchupId=String(formData.get('matchup_id')??'');
  const {data,error}=await supabase.rpc('build_matchup_recap',{p_matchup_id:matchupId});
  if(error) redirect(`/matchups/${matchupId}?error=${encodeURIComponent(error.message)}`);
  revalidatePath(`/matchups/${matchupId}`);
