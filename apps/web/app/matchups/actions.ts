...
import { buildPostgameTalkPrompt, parsePostgameTalkOptions, postgameTalkPromptId, postgameTalkStructuredOutputSchema, type PostgameTalkFacts as Facts, type PostgameTalkTone as Tone } from '../../lib/matchups/postgameTalk';

export async function refreshMatchup(formData: FormData) {
  const supabase = await createClient();
  const matchupId = String(formData.get('matchup_id') ?? '');
  const { error } = await supabase.rpc('recompute_matchup', {
...
function extractResponseText(payload: unknown): string | null {
  ...
  if (Array.isArray(p.output))
    for (const item of p.output as Array<{ content?: unknown }>) {
      ...
      for (const part of item.content as Array<{ text?: unknown }>) {
    ...
    const res = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            role: 'system',
            content: 'Follow the versioned Big Exec postgame prompt. Return only data matching the supplied structured output schema.',
          },
          { role: 'user', content: buildPostgameTalkPrompt(tone, f) },
        ],
        text: {
          format: {
            type: 'json_schema',
            name: 'postgame_talk_options',
            strict: true,
            schema: postgameTalkStructuredOutputSchema,
          },
        },
        metadata: { prompt_id: postgameTalkPromptId },
...
export async function generatePostgameTalk(formData: FormData) {
  ...
  const nameFor = (id: string) => {
    const sf = sfs?.find((x) => x.id === id);
    const rel = sf?.franchises;
    const f = Array.isArray(rel) ? rel[0] : (rel as { name?: string } | null | undefined);
  ...
  const homeName = nameFor(matchup.home_season_franchise_id),
    awayName = nameFor(matchup.away_season_franchise_id),
    winnerName = nameFor(matchup.winner_season_franchise_id);
  ...
  const facts: Facts = {
    ...
    homeName,
    awayName,
    homePoints: Number(matchup.home_points),
    awayPoints: Number(matchup.away_points),
    winnerName,
    loserName: nameFor(loserId),
    margin: Math.abs(Number(matchup.home_points) - Number(matchup.away_points)),
    requesterWon: requesterSf.id === matchup.winner_season_franchise_id,
  };
  const generated = await aiOptions(tone, facts);
  for (const body of generated.options) {
    const { error: recordError } = await supabase.rpc('record_generated_message', {
      p_matchup_id: matchupId,
      p_tone: tone,
      p_body: body,
...
export async function postGeneratedTalk(formData: FormData) {
  ...
  const { data, error } = await supabase.rpc('post_generated_message', {
