...
export default async function PlayersPage({params,searchParams}:{params:Promise<{leagueId:string}>;searchParams:Promise<{position?:string;q?:string}>}){
  ...
  const {data:competitionSeason}=await supabase.from('competition_seasons').select('competition_id').eq('id',season.competition_season_id).maybeSingle();
  const [{data:sfs},{data:rosters},{data:athletes},{data:teams}]=await Promise.all([
    supabase.from('season_franchises').select('id,franchises(name,abbreviation)').eq('league_season_id',season.id),
    supabase.from('roster_entries').select('season_franchise_id,athlete_id,real_team_id').in('season_franchise_id',(await supabase.from('season_franchises').select('id').eq('league_season_id',season.id)).data?.map(x=>x.id)??[]).is('dropped_at',null),
    ...
    competitionSeason?.competition_id?supabase.from('real_teams').select('id,display_name,abbreviation').eq('competition_id',competitionSeason.competition_id).order('abbreviation'):Promise.resolve({data:[]})
  ]);
  ...
  return <main>
    ...
    {(active==='ALL'||active==='D/ST')&&<section className="panel"><p className="eyebrow">TEAM D/ST</p><h2>Defense is a team unit.</h2><div className="playerList">{filteredTeams.map(t=>{const owner=teamOwner.get(t.id);return <article className="playerRow" key={t.id}><div><span>D/ST • {owner?`ROSTERED: ${owner}`:'AVAILABLE'}</span><strong>{t.abbreviation??t.display_name} D/ST</strong></div><b style={{color:owner?'#aaa69d':'#d9b43b',fontSize:'.68rem',letterSpacing:'.08em'}}>{owner?'ROSTERED':'FREE'}</b></article>})}</div></section>}
  </main>;
}
