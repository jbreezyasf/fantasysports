...
export default async function SchedulePage({ params, searchParams }:{params:Promise<{leagueId:string}>;searchParams:Promise<{error?:string;generated?:string}>}) {
  ...
  return <main>
    ...
    <section className="panel"><p className="eyebrow">STANDINGS</p><div className="standingsList" role="table" aria-label="League standings"><div className="srOnly" role="row"><span role="columnheader">Rank</span><span role="columnheader">Team</span><span role="columnheader">Record</span><span role="columnheader">Points for</span><span role="columnheader">Points against</span><span role="columnheader">Streak</span></div>{(standings??[]).map((s,index)=>{const f=franchiseMap.get(s.season_franchise_id);const record=`${s.wins}-${s.losses}${s.ties?`-${s.ties}`:''}`;return <div className="standingRow" role="row" aria-label={standingRowLabel({rank:index+1,team:f?.name??'Franchise',record,pointsFor:Number(s.points_for),pointsAgainst:Number(s.points_against),streak:s.streak})} key={s.season_franchise_id}><b role="cell">{index+1}</b><strong role="cell">{f?.name??'Franchise'}</strong><span role="cell">{record}</span><small role="cell">{Number(s.points_for).toFixed(1)} PF</small><small className="srOnly" role="cell">{Number(s.points_against).toFixed(1)} PA</small><small className="srOnly" role="cell">{s.streak??'No streak'}</small></div>})}</div></section>

    {!!seeds?.length&&<section className="panel"><p className="eyebrow">POSTSEASON FIELD</p><h2>Seeds are locked.</h2><div className="standingsList" role="table" aria-label="Postseason seeds"><div className="srOnly" role="row"><span role="columnheader">Seed</span><span role="columnheader">Team</span><span role="columnheader">Bracket</span></div>{seeds.map(s=>{const f=franchiseMap.get(s.season_franchise_id);const bracket=s.bracket==='championship'?'TITLE FIELD':'REDEMPTION';return <div className="standingRow" role="row" aria-label={`Seed ${s.seed}. Team ${f?.name??'Franchise'}. Bracket ${bracket}.`} key={s.season_franchise_id}><b role="cell">#{s.seed}</b><strong role="cell">{f?.name??'Franchise'}</strong><span role="cell">{bracket}</span></div>})}</div>{champion&&<p className="successNotice">League Champion: {franchiseMap.get(champion.winner_season_franchise_id)?.name??'Champion'}</p>}{redemption&&<p className="successNotice">Redemption Champion: {franchiseMap.get(redemption.winner_season_franchise_id)?.name??'Champion'}</p>}</section>}
